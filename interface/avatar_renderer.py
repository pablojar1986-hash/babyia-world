...
class AvatarRenderer:
    ...
    def draw(
        ...
        if level >= 1:
            ...
            if frust > 0.6:
                pygame.draw.arc(
                    surface, _DARK, pygame.Rect(cx - 4, cy + 2, 8, 6), 0, 3.14, 1
                )
            else:
                pygame.draw.arc(
                    surface, _DARK, pygame.Rect(cx - 4, cy + 3, 8, 5), 3.14, 6.28, 1
                )

        if level >= 3:
            # Mochila simbolica (pequeno rectangulo)
            pygame.draw.rect(
                surface, (100, 80, 60), pygame.Rect(cx + r - 4, cy - 3, 5, 6)
            )

        if level >= 5 and energy > 0.8:
            # Aura de energia
            for ddx, ddy in [(-r - 3, 0), (r + 3, 0), (0, -r - 3)]:
                pygame.draw.circle(surface, (255, 240, 100), (cx + ddx, cy + ddy), 2)
