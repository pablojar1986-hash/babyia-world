"""Fabrica de laberintos progresivos por nivel para BabyIA World 0.4.5."""
...
def get_maze_for_level(level: int) -> dict:
    ...
    Nivel 0: mundo completamente abierto (sin paredes), 8x8.
    Nivel 1: paredes fijas BASE_WALLS, 8x8.
    Niveles 2+: generacion procedural con grid escalado.
    """
    ...
    _start = get_start_pos(grid_size)
    _goal = get_progress_door_pos(grid_size)
    ...
    if level == 0:
        ...
        solvable = is_solvable(walls, _start, _goal, grid_size)
    elif requires_key_door:
        walls, seed = generate_solvable_maze_with_key_door(
            base_seed, density, grid_size=grid_size
        )
        solvable = is_solvable_with_key_door(walls, grid_size=grid_size)
    else:
        walls, seed = generate_solvable_maze(base_seed, density, grid_size=grid_size)
        solvable = is_solvable(walls, _start, _goal, grid_size)
...
def save_level_stats(maze_info: dict, stats_file: Path | None = None):
    ...
    payload = {
        k: (sorted(v) if isinstance(v, frozenset) else v) for k, v in maze_info.items()
    }
