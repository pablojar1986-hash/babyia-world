...
class PreferenceTracker:
    ...
    def update(self, world_id: str, reward: float, risk_events: int,
               returned_home: bool, steps: int):
        if world_id not in self._raw:
            self._raw[world_id] = {
                "visits": 0, "total_reward": 0.0,
                "total_risk": 0, "total_returned": 0, "total_steps": 0,
            }
    ...
    def get_score(self, world_id: str) -> float:
        ...
        return avg_r + ret_r * RETURN_WEIGHT - avg_ri * RISK_WEIGHT - avg_s * STEPS_FACTOR
