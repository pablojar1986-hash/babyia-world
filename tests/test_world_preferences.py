...
def test_update_and_get_score():
    pt = make_tracker()
    pt.update("food_world", reward=5.0, risk_events=0,
               returned_home=True, steps=30)
...
def test_preference_formula_components():
    ...
    pt.update("food_world", reward=10.0, risk_events=0,
               returned_home=True, steps=20)
    score = pt.get_score("food_world")
...
def test_high_risk_lowers_score():
    pt = make_tracker()
    pt.update("danger", reward=5.0, risk_events=10,
               returned_home=True, steps=40)
...
def test_best_world_selection():
    pt = make_tracker()
    pt.update("food_world",  reward=8.0, risk_events=0,
               returned_home=True, steps=20)
    pt.update("danger_world", reward=2.0, risk_events=5,
               returned_home=False, steps=50)
    best = pt.best_world()
